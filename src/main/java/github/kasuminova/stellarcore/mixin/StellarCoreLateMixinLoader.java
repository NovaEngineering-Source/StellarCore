...
public class StellarCoreLateMixinLoader implements ILateMixinLoader {
    ...
    public boolean shouldMixinConfigQueue(final String mixinConfig) {
        ...
        if (supplier == null) {
            StellarLog.LOG.warn("[StellarCore-MixinLoader] Mixin config {} is not found in config map! It will never be loaded.", mixinConfig);
            return false;
        }
        boolean shouldLoad = supplier.getAsBoolean();
        if (!shouldLoad) {
            StellarLog.LOG.info("[StellarCore-MixinLoader] Mixin config {} is disabled by config or mod is not loaded.", mixinConfig);
        }
        return shouldLoad;
    }
