...
public class ContainerTECache {
    ...
    private static final Map<Class<? extends Container>, Function<Container, List<TileEntity>>> CACHE = new java.util.concurrent.ConcurrentHashMap<>();
    private static final Map<Class<?>, Map<Class<?>, List<Field>>> CLASS_FILED_TYPE_CACHE = new java.util.concurrent.ConcurrentHashMap<>();
    ...
    private static List<Field> scanTileEntityFieldRecursive(Class<?> aClass, Class<?> target) {
        Map<Class<?>, List<Field>> cachedFieldMap = CLASS_FILED_TYPE_CACHE.computeIfAbsent(aClass, v -> new java.util.concurrent.ConcurrentHashMap<>());
